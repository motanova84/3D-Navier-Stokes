...
class UniversalValidator:
    ...
    def plot_summary(self, results: List[Dict], 
        ...
        for r in results:
            ...
            snr_str = f"{snr:.1f}" if snr not in ['N/A', None] else "N/A"
            sig_str = f"{sig:.1f}σ" if sig not in ['N/A', None] else "N/A"
...
"""
        ...
        # Conclusión
        n_detected = sum(1 for r in results if r.get('significance_sigma') is not None and r.get('significance_sigma', 0) > 3)
        n_total = len(results)
