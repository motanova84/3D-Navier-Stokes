...
# ═══════════════════════════════════════════════════════════════
# FUNCIÓN REUTILIZABLE PARA BARRIDOS PARAMÉTRICOS
# ═══════════════════════════════════════════════════════════════

def run_psi_nse_simulation(N=128, L=2*np.pi, T=5.0, nu=1e-3, f0=141.7001, 
                          dt=0.001, save_interval=0.1, verbose=False):
    """
    Ejecuta una simulación Ψ-NSE con parámetros especificados.
    
    Esta función encapsula toda la lógica de simulación del script principal
    pero permite ejecutarla con diferentes parámetros de manera programática.
    
    Args:
        N: Resolución espacial (puntos por dimensión)
        L: Tamaño del dominio (periódico)
        T: Tiempo total de simulación
        nu: Viscosidad cinemática
        f0: Frecuencia base del campo Ψ (Hz)
        dt: Paso temporal
        save_interval: Intervalo para guardar snapshots (segundos)
        verbose: Si True, imprime progreso
    
    Returns:
        dict: Resultados de la simulación con métricas y diagnósticos incluyendo:
            - parameters: Diccionario de parámetros de entrada
            - time, energy, enstrophy: Series temporales (listas)
            - detected_frequency_Hz: Frecuencia dominante detectada
            - final_energy, final_enstrophy: Valores finales
            - max_energy, min_energy: Valores extremos
            - blow_up_detected: Boolean indicando si se detectó blow-up
            - frequency_error_percent: Error relativo en frecuencia detectada
    """
    
    if verbose:
        print(f"🌊 Ejecutando simulación Ψ-NSE")
        print(f"   N={N}, f0={f0:.2f} Hz, Re={L/nu:.1f}, T={T}s")
    
    # Configuración de coeficientes
    omega0_sim = 2 * np.pi * f0
    alpha_coupling_sim = 1/(720 * np.pi**2) * 0.1
    gamma_coupling_sim = -1/(1440 * np.pi**2)
    
    # Grid espectral
    x_sim = np.linspace(0, L, N, endpoint=False)
    dx_sim = x_sim[1] - x_sim[0]
    X_sim, Y_sim, Z_sim = np.meshgrid(x_sim, x_sim, x_sim, indexing='ij')
    
    # Frecuencias espectrales
    kx_sim = fftfreq(N, dx_sim/(2*np.pi))
    ky_sim = fftfreq(N, dx_sim/(2*np.pi))
    kz_sim = fftfreq(N, dx_sim/(2*np.pi))
    KX_sim, KY_sim, KZ_sim = np.meshgrid(kx_sim, ky_sim, kz_sim, indexing='ij')
    K2_sim = KX_sim**2 + KY_sim**2 + KZ_sim**2
    K2_sim[0,0,0] = 1
    
    # Campo de coherencia Ψ
    def campo_coherencia_psi_sim(t):
        temporal = np.sin(omega0_sim * t)
        espacial = (np.sin(2*np.pi*X_sim/L) * 
                    np.cos(2*np.pi*Y_sim/L) * 
                    np.sin(2*np.pi*Z_sim/L))
        return temporal * espacial
    
    # Tensor de acoplamiento Φ_ij
    def calcular_phi_tensor_sim(psi):
        psi_hat = fftn(psi)
        laplacian_psi = np.real(ifftn(-K2_sim * psi_hat))
        d2_dx2 = np.real(ifftn(-KX_sim**2 * psi_hat))
        d2_dy2 = np.real(ifftn(-KY_sim**2 * psi_hat))
        d2_dz2 = np.real(ifftn(-KZ_sim**2 * psi_hat))
        d2_dxdy = np.real(ifftn(-KX_sim*KY_sim * psi_hat))
        d2_dxdz = np.real(ifftn(-KX_sim*KZ_sim * psi_hat))
        d2_dydz = np.real(ifftn(-KY_sim*KZ_sim * psi_hat))
        
        Phi = np.zeros((3, 3, N, N, N))
        Phi[0,0] = alpha_coupling_sim * d2_dx2 + gamma_coupling_sim * laplacian_psi
        Phi[1,1] = alpha_coupling_sim * d2_dy2 + gamma_coupling_sim * laplacian_psi
        Phi[2,2] = alpha_coupling_sim * d2_dz2 + gamma_coupling_sim * laplacian_psi
        Phi[0,1] = Phi[1,0] = alpha_coupling_sim * d2_dxdy
        Phi[0,2] = Phi[2,0] = alpha_coupling_sim * d2_dxdz
        Phi[1,2] = Phi[2,1] = alpha_coupling_sim * d2_dydz
        return Phi
    
    # Proyección de incompresibilidad
    def proyectar_divergence_free_sim(u_hat, v_hat, w_hat):
        kdotu = KX_sim*u_hat + KY_sim*v_hat + KZ_sim*w_hat
        u_hat = u_hat - KX_sim * kdotu / K2_sim
        v_hat = v_hat - KY_sim * kdotu / K2_sim
        w_hat = w_hat - KZ_sim * kdotu / K2_sim
        return u_hat, v_hat, w_hat
    
    # RHS de Ψ-NSE
    def rhs_psi_nse_sim(u, v, w, Phi):
        u_hat = fftn(u)
        v_hat = fftn(v)
        w_hat = fftn(w)
        
        # Términos no lineales
        ux = np.real(ifftn(1j * KX_sim * u_hat))
        uy = np.real(ifftn(1j * KY_sim * u_hat))
        uz = np.real(ifftn(1j * KZ_sim * u_hat))
        vx = np.real(ifftn(1j * KX_sim * v_hat))
        vy = np.real(ifftn(1j * KY_sim * v_hat))
        vz = np.real(ifftn(1j * KZ_sim * v_hat))
        wx = np.real(ifftn(1j * KX_sim * w_hat))
        wy = np.real(ifftn(1j * KY_sim * w_hat))
        wz = np.real(ifftn(1j * KZ_sim * w_hat))
        ...
        conv_u_hat, conv_v_hat, conv_w_hat = proyectar_divergence_free_sim(
            conv_u_hat, conv_v_hat, conv_w_hat
        )
        coupling_u_hat, coupling_v_hat, coupling_w_hat = proyectar_divergence_free_sim(
            coupling_u_hat, coupling_v_hat, coupling_w_hat
        )
        
        du_hat = conv_u_hat - nu*K2_sim*u_hat + coupling_u_hat
        dv_hat = conv_v_hat - nu*K2_sim*v_hat + coupling_v_hat
        dw_hat = conv_w_hat - nu*K2_sim*w_hat + coupling_w_hat
        return du_hat, dv_hat, dw_hat
    
    # Condiciones iniciales: Taylor-Green
    u_sim = np.sin(X_sim) * np.cos(Y_sim) * np.cos(Z_sim)
    v_sim = -np.cos(X_sim) * np.sin(Y_sim) * np.cos(Z_sim)
    w_sim = np.zeros_like(u_sim)
    
    u_hat_sim = fftn(u_sim)
    v_hat_sim = fftn(v_sim)
    w_hat_sim = fftn(w_sim)
    ...
    if verbose:
        from tqdm import tqdm
        iterator = tqdm(iterator, desc="Simulando")
    ...
    for step in iterator:
        ...
        psi_sim = campo_coherencia_psi_sim(t)
        Phi_sim = calcular_phi_tensor_sim(psi_sim)
        
        # RK4
        k1_u, k1_v, k1_w = rhs_psi_nse_sim(u_sim, v_sim, w_sim, Phi_sim)
        u1 = np.real(ifftn(u_hat_sim + 0.5*dt*k1_u))
        v1 = np.real(ifftn(v_hat_sim + 0.5*dt*k1_v))
        w1 = np.real(ifftn(w_hat_sim + 0.5*dt*k1_w))
        
        k2_u, k2_v, k2_w = rhs_psi_nse_sim(u1, v1, w1, Phi_sim)
        u2 = np.real(ifftn(u_hat_sim + 0.5*dt*k2_u))
        v2 = np.real(ifftn(v_hat_sim + 0.5*dt*k2_v))
        w2 = np.real(ifftn(w_hat_sim + 0.5*dt*k2_w))
        
        k3_u, k3_v, k3_w = rhs_psi_nse_sim(u2, v2, w2, Phi_sim)
        u3 = np.real(ifftn(u_hat_sim + dt*k3_u))
        v3 = np.real(ifftn(v_hat_sim + dt*k3_v))
        w3 = np.real(ifftn(w_hat_sim + dt*k3_w))
        
        k4_u, k4_v, k4_w = rhs_psi_nse_sim(u3, v3, w3, Phi_sim)
        
        u_hat_sim = u_hat_sim + (dt/6)*(k1_u + 2*k2_u + 2*k3_u + k4_u)
        v_hat_sim = v_hat_sim + (dt/6)*(k1_v + 2*k2_v + 2*k3_v + k4_v)
        w_hat_sim = w_hat_sim + (dt/6)*(k1_w + 2*k2_w + 2*k3_w + k4_w)
        
        u_hat_sim, v_hat_sim, w_hat_sim = proyectar_divergence_free_sim(
            u_hat_sim, v_hat_sim, w_hat_sim
        )
        
        u_sim = np.real(ifftn(u_hat_sim))
        v_sim = np.real(ifftn(v_hat_sim))
        w_sim = np.real(ifftn(w_hat_sim))
        
        # Diagnósticos
        energia[step] = 0.5 * np.mean(u_sim**2 + v_sim**2 + w_sim**2)
        
        # Vorticidad
        omega_x = np.real(ifftn(1j*KY_sim*w_hat_sim - 1j*KZ_sim*v_hat_sim))
        omega_y = np.real(ifftn(1j*KZ_sim*u_hat_sim - 1j*KX_sim*w_hat_sim))
        omega_z = np.real(ifftn(1j*KX_sim*v_hat_sim - 1j*KY_sim*u_hat_sim))
    ...
    return {
        ...
        'time': tiempo.tolist(),
        'energy': energia.tolist(),
        'enstrophy': enstrofia.tolist(),
        ...
        'min_energy': float(np.min(energia)),
        'blow_up_detected': bool(np.max(energia) > 1e6),
        'frequency_error_percent': float(abs(f_detected - f0)/f0 * 100)
    }

# ═══════════════════════════════════════════════════════════════
# SCRIPT PRINCIPAL (solo ejecuta cuando se invoca directamente)
# ═══════════════════════════════════════════════════════════════
